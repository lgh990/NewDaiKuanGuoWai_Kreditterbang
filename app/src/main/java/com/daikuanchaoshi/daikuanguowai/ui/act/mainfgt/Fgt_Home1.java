...
public class Fgt_Home1 extends BaseFragment implements Dilog_TestingDate.OnBackCenter {
    ...
    Unbinder unbinder1;
    private static boolean homeStatus = false;
            ...
            public void onSucceed(String succeed) {
                ...
                try {
                    ...
                    if (entity.getStatus() == 1) {
                         ...
                         * 处理贷款拒绝页显示
                         */
                        if (entity.getData().getData().getLoan() != null && entity.getData().getData().getLoan().getStatus().equals("1") && entity.getData().getData().getLoan().getShenhestatus().equals("0")) {//走获取额度
                            llCard.setVisibility(View.GONE);
                            llCard1.setVisibility(View.VISIBLE);
                            myVip.setVisibility(View.VISIBLE);
                            ll_Banner.setVisibility(View.GONE);
                            allPrice.setPaintFlags(allPrice.getPaintFlags() | Paint.STRIKE_THRU_TEXT_FLAG);
                            allPrice1.setText(entity.getData().getData().getLoan().getH5_money_one());
                            allPrice.setText(entity.getData().getData().getLoan().getH5_money_two());
...
//            Toast.makeText(getActivity(), "请先完善认证资料", Toast.LENGTH_SHORT).show();
    ...
    public void postapplyLoan() {
        HttpHelper.loanapplyLoan(getContext(), "1", entity.getData().getData().getAmount_limit(), new HttpHelper.HttpUtilsCallBack<String>() {
            ...
            public void onSucceed(String succeed) {
                loding.dismiss();
                Gson gson = new Gson();
                postapplyLoanBean = gson.fromJson(succeed, com.daikuanchaoshi.daikuanguowai.ui.act.mainfgt.home.bean.postapplyLoanBean.class);
                if (entity.getStatus() == 1) {
                    if (MyApplication.getLognBean() != null) {
                        homeDashbord(MyApplication.getLognBean().getMember().getToken());
                    } else {
    ...
    @OnClick({R.id.bt_tvtv, R.id.view_desc, R.id.xieyiRadio, R.id.userXieyi, R.id.myVip1, R.id.myVip, R.id.hoee_but})
